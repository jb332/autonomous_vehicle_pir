...
def creationAE_DATA(nameAE):
	...
	createAE(url, "app_"+nameAE+"_Sensor" , nameAE, "[Type/sensor]")
	createContainer("admin:admin", url+"/"+nameAE, "DATA", "data")


def init_om2m(nameAE, port):
	url = "http://localhost:8080/~/in-cse/in-name"
	createAE(url, "GPS_Sensor", nameAE, "test")
	creationAE_DATA(nameAE)
	Listenurl = "http://localhost:"+str(port)
	subToAE("admin:admin", Listenurl, "Sub" + nameAE, url+"/"+nameAE+"/DATA")
